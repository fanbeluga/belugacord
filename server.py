...
CHANGELOG={"1.8":{"title":"Belugacord Beta 1.8","items":["✅ Друзья переписаны","😈 ADMIN ABUSE: рандомайзеры + планировщик","🤝 Кооп-абьюз","🚨 Бан-заявки от админов","📊 Suspicious-логи","🎮 Новые игры: Flappy, Тетрис, Memory, Реакция","🏆 Турниры","🖥️ Звонки с трансляцией экрана","⏱️ Таймер абьюза у всех"]},"1.7":{"title":"Belugacord Beta 1.7","items":["📞 Звонки","📑 Табы","🟢 Онлайн-кружок","🎁 Кастом подарки","🎨 NFT с картинками"]},"1.6":{"title":"Belugacord Beta 1.6","items":["🔍 Поиск","👥 Друзья","⚙️ Настройки сервера","📱 Мобилка"]},"0.6":{"title":"Belugacord 0.6","items":["Первый релиз","Темы, магазин, NFT"]}}
...
async def update_profile(data:dict):
    ...
    p=await get_pool()
    async with p.acquire() as conn:
        await conn.execute("""UPDATE users SET avatar=COALESCE($1,avatar),banner=COALESCE($2,banner),avatar_pos=COALESCE($3,avatar_pos),banner_pos=COALESCE($4,banner_pos),nickname_color=$5,nickname_gradient=$6,bio=COALESCE($7,bio),fav_music=COALESCE($8,fav_music) WHERE id=$9""",data.get("avatar"),data.get("banner"),data.get("avatar_pos"),data.get("banner_pos"),data.get("nickname_color"),data.get("nickname_gradient"),data.get("bio"),data.get("fav_music"),user["id"])
    return {"ok":True}
...
async def get_user(user_id:int):
    p=await get_pool()
    async with p.acquire() as conn:
        row=await conn.fetchrow("""SELECT id,username,avatar,banner,avatar_pos,banner_pos,is_admin,is_moderator,is_beta_tester,is_scam,is_dev,premium_tier,nickname_color,nickname_gradient,bio,fav_music,messages_count,is_legend,created_at,last_seen FROM users WHERE id=$1""",user_id)
...
async def users_search(q:str,token:str):
    ...
    p=await get_pool()
    async with p.acquire() as conn:
        rows=await conn.fetch("SELECT id,username,avatar,is_admin,is_moderator,is_beta_tester,is_scam,is_dev FROM users WHERE username ILIKE $1 AND id!=$2 ORDER BY username LIMIT 20",f"%{q}%",user["id"])
    out=[]
    for r in rows:
...
async def admin_action(data:dict):
    ...
    if action=="ban" and not is_owner: raise HTTPException(403,"Отправь заявку через /api/ban_requests/submit")
...
async def admin_logs(token:str):
    ...
    p=await get_pool()
    async with p.acquire() as conn:
        rows=await conn.fetch("SELECT l.id,l.action,l.created_at,a.username AS admin_name FROM admin_logs l LEFT JOIN users a ON a.id=l.admin_id ORDER BY l.id DESC LIMIT 100")
    out=[]
    for r in rows:
...
async def owner_create_gift(data:dict):
    ...
    async with p.acquire() as conn:
        ...
        await conn.execute("INSERT INTO custom_gifts(gift_id,name,emoji,image,price) VALUES($1,$2,$3,$4,$5)",gid,name,data.get("emoji","🎁"),data.get("image"),price)
    return {"ok":True}
...
async def friends_list(token:str):
    ...
    async with p.acquire() as conn:
        ...
        for r in frows:
            ...
            o=await conn.fetchrow("SELECT id,username,avatar,is_admin,is_moderator,is_beta_tester,is_scam,is_dev,last_seen FROM users WHERE id=$1",oid)
            if not o: continue
            result.append({"id":o["id"],"username":o["username"],"avatar":o["avatar"],"status":"accepted","friend_row_id":r["id"],"online":o["id"] in online_users,"last_seen":o["last_seen"].isoformat() if o.get("last_seen") else None,"is_admin":o["is_admin"],"is_moderator":o["is_moderator"],"is_beta_tester":o["is_beta_tester"],"is_scam":o["is_scam"],"role":get_role(o)})
        for r in inc:
...
async def server_update(data:dict):
    ...
    async with p.acquire() as conn:
        ...
        await conn.execute("UPDATE servers SET name=COALESCE($1,name),description=COALESCE($2,description) WHERE id=$3",data.get("name"),data.get("description"),int(data.get("server_id",0)))
    return {"ok":True}
...
async def channel_messages(channel_id:int,token:str):
    ...
    p=await get_pool()
    async with p.acquire() as conn:
        rows=await conn.fetch("SELECT m.id,m.text,m.file_url,m.reactions,m.created_at,m.user_id,u.username,u.avatar,u.avatar_pos,u.is_admin,u.is_moderator,u.is_beta_tester,u.is_scam,u.is_dev FROM messages m JOIN users u ON u.id=m.user_id WHERE m.channel_id=$1 ORDER BY m.id ASC LIMIT 200",channel_id)
    out=[]
    for r in rows:
        d=dict(r)
        d["created_at"]=d["created_at"].isoformat() if d.get("created_at") else None
    ...
    try:
        while True:
            ...
            if t=="message":
                ...
                p=await get_pool()
                async with p.acquire() as conn:
                    msg=await conn.fetchrow("INSERT INTO messages(channel_id,user_id,text,file_url) VALUES($1,$2,$3,$4) RETURNING *",int(ch),uid,text,furl)
                ...
                payload={"type":"message","id":msg["id"],"channel_id":int(ch),"user_id":uid,"username":user["username"],"avatar":user.get("avatar"),"avatar_pos":user.get("avatar_pos"),"text":text,"file_url":furl,"created_at":msg["created_at"].isoformat(),"is_admin":user.get("is_admin"),"is_moderator":user.get("is_moderator"),"is_beta_tester":user.get("is_beta_tester"),"is_scam":user.get("is_scam"),"role":get_role(user),"temp_id":tid}
...
async def manifest():
    return {"name":"Belugacord Beta 1.8","short_name":"Belugacord","start_url":"/","display":"standalone","background_color":"#0a0a12","theme_color":"#0a0a12","icons":[{"src":"/uploads/icon.png","sizes":"192x192","type":"image/png"}]}
