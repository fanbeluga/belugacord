...
async def get_current_user(token: str):
    ...
    p = await get_pool()
    async with p.acquire() as conn:
        row = await conn.fetchrow("SELECT id, username, avatar, banner FROM users WHERE id = $1", user_id)
...
async def register(data: dict):
    ...
    async with p.acquire() as conn:
        ...
        row = await conn.fetchrow(
            "INSERT INTO users (username, password_hash) VALUES ($1, $2) RETURNING id, username, avatar, banner",
...
async def login(data: dict):
    ...
    p = await get_pool()
    async with p.acquire() as conn:
        row = await conn.fetchrow("SELECT id, username, password_hash, avatar, banner FROM users WHERE username = $1", username)
    if not row or not verify_password(password, row["password_hash"]):
        raise HTTPException(400, "Неверный ник или пароль")
    token = make_token(row["id"], row["username"])
    return {"token": token, "user": {"id": row["id"], "username": row["username"], "avatar": row["avatar"], "banner": row["banner"]}}
...
async def update_profile(data: dict):
    ...
    p = await get_pool()
    async with p.acquire() as conn:
        await conn.execute("UPDATE users SET avatar = COALESCE($1, avatar), banner = COALESCE($2, banner) WHERE id = $3",
                           avatar, banner, user["id"])
    return {"ok": True}
...
async def friends_list(token: str):
    ...
    p = await get_pool()
    async with p.acquire() as conn:
        rows = await conn.fetch("""SELECT u.id, u.username, u.avatar, f.status FROM friendships f
            JOIN users u ON (u.id = f.from_user OR u.id = f.to_user)
        ...
        while True:
            ...
            if t == "message":
                ...
                await manager.broadcast({"type": "message", "id": row["id"], "channel_id": channel_id,
                    "username": user["username"], "avatar": user["avatar"], "text": text,
                    "file_url": file_url, "created_at": row["created_at"].isoformat()})
            elif t in ("call", "offer", "answer", "ice"):
                target = data.get("target")
                if target:
                    await manager.send_to(target, {**data, "from": user["id"], "from_name": user["username"]})
